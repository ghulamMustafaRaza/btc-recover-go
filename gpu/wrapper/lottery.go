// Package wrapper provides GPU-accelerated BTC recover operations.
package wrapper
...
// RecoverKernel manages the GPU kernel for BTC address matching.
...
// RecoverConfig configures the recover kernel.
type RecoverConfig struct {
	PTXPath     string // Path to btc_recover.ptx
...
// NewRecoverKernel creates a new GPU recover kernel.
func NewRecoverKernel(device *Device, config RecoverConfig) (*RecoverKernel, error) {
	...
	// Get kernel function
	kernel, err := module.GetFunction("btc_recover_kernel")
	if err != nil {
...
func (lk *RecoverKernel) LoadGTable(xPath, yPath string) error {
...
func (lk *RecoverKernel) LoadGTableFromBytes(xData, yData []byte) error {
...
func (lk *RecoverKernel) LoadHashTable(hashes []uint64) error {
	// Ensure context is current
	if err := lk.device.SetCurrent(); err != nil {
		return fmt.Errorf("failed to set context: %w", err)
	}
...
func (lk *RecoverKernel) allocateBatchBuffers() error {
	var err error
...
func (lk *RecoverKernel) ProcessBatch(privKeys []byte) ([]Match, error) {
...
func (lk *RecoverKernel) BatchSize() int {
	return lk.batchSize
}

// Close releases all GPU resources.
func (lk *RecoverKernel) Close() error {
	if lk.gTableX != nil {
		lk.gTableX.Free()
	}
	if lk.gTableY != nil {
