...
import (
	...
	"btc_recover/gpu/gtable"
	"btc_recover/gpu/wrapper"
...
func NewGPUWorker(hashSet *lookup.AddressHashSet, cfg GPUWorkerConfig) (*GPUWorker, error) {
	...
	// Create recover kernel
	recoverConfig := wrapper.RecoverConfig{
		PTXPath:   cfg.PTXPath,
		BatchSize: cfg.GPUBatchSize * 80, // 80 addresses per mnemonic (4 types × 20 indexes)
	}

	recover, err := wrapper.NewRecoverKernel(device, recoverConfig)
	if err != nil {
		device.Close()
		return nil, fmt.Errorf("creating recover kernel: %w", err)
	}

	// Load GTable (generate if not provided)
	if cfg.GTableXPath != "" && cfg.GTableYPath != "" {
		if err := recover.LoadGTable(cfg.GTableXPath, cfg.GTableYPath); err != nil {
			recover.Close()
			device.Close()
		...
		if err := recover.LoadGTableFromBytes(gt.X, gt.Y); err != nil {
			recover.Close()
			device.Close()
	...
	if err := recover.LoadHashTable(hashes); err != nil {
		recover.Close()
		device.Close()
...
func (w *GPUWorker) processBatchLocked() []Match {
	...
	gpuMatches, err := w.recover.ProcessBatch(w.pendingPrivs)
	if err != nil {
