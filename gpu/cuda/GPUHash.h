 ...
 *
 * Trimmed down for btc_recover - removed CudaBrainSecp specific functions.
 */
...
__device__ void _SHA256Transform(uint32_t output[8], uint32_t* w)
{
...
__device__ void _RIPEMD160Transform(uint32_t s[5], uint32_t* w)
{
...
// x: X coordinate of public key (256 bits / 4 uint64)
// isOdd: parity of Y coordinate (0 = even/02, 1 = odd/03)
// hash: output 20-byte hash
__device__ __noinline__ void _GetHash160Comp(uint64_t* x, uint8_t isOdd, uint8_t* hash)
{
	...
	// Compressed public key: 02/03 + X (33 bytes total)
	...
	publicKeyBytes[13] = 0;
	publicKeyBytes[14] = 0;
	publicKeyBytes[15] = 0x108;  // 33 * 8 = 264 = 0x108
...
// x, y: X and Y coordinates of public key (256 bits / 4 uint64 each)
// hash: output 20-byte hash
__device__ __noinline__ void _GetHash160(uint64_t* x, uint64_t* y, uint8_t* hash)
{
	...
	// Uncompressed public key: 04 + X + Y (65 bytes total)
	...
	publicKeyBytes[29] = 0;
	publicKeyBytes[30] = 0;
	publicKeyBytes[31] = 0x208;  // 65 * 8 = 520 = 0x208

	_SHA256Initialize(s);
	_SHA256Transform(s, publicKeyBytes);
	_SHA256Transform(s, publicKeyBytes + 16);
